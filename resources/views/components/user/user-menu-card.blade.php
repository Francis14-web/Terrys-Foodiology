@props([
    ...
    <div class="cursor-pointer m-4 w-fit h-fit bg-white flex items-center justify-evenly flex-col shadow-md rounded-3xl hover:shadow-none hover:bg-green-400"
        @if ($food->food_stock > 0) wire:click="openFood('{{$food->id}}')" @endif>
        <div class="relative flex  w-full h-full overflow-hidden">
            <img src="{{ asset('storage/'.$lastImagePath) }}" class="{{(!$food->food_stock) ? 'blur-sm' : ''}} h-full w-full object-cover rounded-3xl">
